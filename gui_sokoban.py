...
# move actions
direction_offset = {'Left': (-1, 0), 'Right': (1, 0),
                    'Up': (0, -1), 'Down': (0, 1)}  # (x,y) = (column,row)
...
def start_level():
    ...
    root_window.title(
        f'Sokoban v{__version__} - {warehouse_path.split("/")[-1]}')
...
def move_player(direction):
    ...
    # where the player will go if possible
...
def try_move_box(location, next_location):
    ...
    if (next_x, next_y) not in warehouse.walls and (next_x, next_y) not in warehouse.boxes:
        ...
        if (next_x, next_y) in warehouse.targets:
            cells[(next_x, next_y)] = make_cell(
                'box_on_target', get_box_weight(x, y))
        else:
